...
Markdown 파일을 고르면 앱 변경을 결정론적 보기로 자동 생성하며, 이 연결도 현재 페이지
세션에만 유지됩니다. 충돌 해결에서 그 정본 문서의 기존 항목 제한 편집을 개수로 미리 보고
검증해 가져올 수 있습니다. 빌드 없이 정적 파일 그대로 올립니다.
...
- **Markdown 연결** — Obsidian vault에서 고른 `.md` 파일에 현재 목록을 즉시 생성하고 앱의
  ...
  Markdown read→verify→write 사이 TOCTOU는 남는 best-effort 보호입니다. 충돌 해결의 가져오기는
  ...
  재배치를 지원하고, 항목 추가·삭제·카테고리 CRUD·기타 절·설정·임의 Markdown 변경은 거부합니다.
  미리보기와 확인 사이 앱 snapshot이 달라지면 덮어쓰지 않습니다. 확인 성공은 Store/LocalStorage를
  먼저 적용하고 JSON mirror와 Markdown 정본 저장을 독립 처리합니다. Markdown 정본 저장 실패에도
  적용한 LocalStorage는 롤백하지 않고 충돌을 유지합니다
- **뽀모도로 타이머** — 집중·휴식 4회차 사이클과 단일 타이머. 회차별 시간을 직접 정하고,
  펼치면 시간이 흐른 만큼 채워지는 원형 시계로 봅니다. 새로고침해도 돌아가던 자리에서 이어집니다.
  한 구간이 끝나면 `00:00`에 멈춰 서서 `휴식 시작` 버튼을 내밀고 기다립니다 — 저절로
  넘어가면 자리를 비운 사이에 경계가 지나가버려, 돌아왔을 때 흐르는 숫자만 남습니다.
  패널을 닫으면 화면 오른쪽 아래에 반투명한 미니 타이머가 남습니다 — 얼마나 진하게
  보일지는 설정에서 정하고 그 값은 저장됩니다. Chrome·Edge에서는 항상 위에 뜨는 별도
  창으로 빼서 브라우저를 최소화해도 볼 수 있습니다 — 시간이 흐른 만큼 채워지는
  원형 시계 안에 남은 시간과 버튼이 함께 들어 있어 창을 작게 두어도 되고,
  숫자만 보는 방식으로 바꿀 수도 있습니다
- **다크 모드** — OS 설정을 따르다가, 고르면 그 선택을 지킵니다
- **실행 취소** — 삭제하면 5초 동안 되돌릴 수 있습니다
- **문의하기** — 제목과 내용을 적으면 그 내용이 채워진 채로 메일 앱이 열립니다. 이 앱이 직접
  보내지는 않습니다 — `connect-src 'none'`이라 밖으로 보낼 길 자체가 없어, 보내는 일은
  브라우저가 메일 앱에 넘깁니다. 할 일 내용은 함께 담기지 않습니다
...
- 성능: 조회는 `Map` 색인으로 항목 수에 비례하게 유지하고, 판 번호와 테마는 저장본
  앞머리만 읽어 첫 페인트를 막지 않습니다. 1초마다 도는 뽀모도로는 접혀서 보이지 않는
  자리를 건너뛰어, 한 틱에 문서 제목과 지금 보이는 시계 두 곳만 갱신합니다. 목표는 항목 100개 기준 초기 로드 200ms,
...
- [`docs/todo-app-prd.md`](docs/todo-app-prd.md) — 기능 요구사항 23개(F-01~F-23), 데이터 모델,
